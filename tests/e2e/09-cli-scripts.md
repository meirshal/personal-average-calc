...
- Run `bun run db:add-admin` with a new email and school slug
...
- Run `bun run db:add-admin` with the same email and school B's slug
...
- Run `bun run db:add-admin` with the same email and same school slug
...
- Run `bun run db:remove-admin` with `--school` flag specifying school A
...
- Run `bun run db:remove-admin` without `--school` flag
...
- Run `bun run db:list-schools` (via `scripts/list-schools.ts`)
